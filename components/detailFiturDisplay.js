import DetailFitur from '../data/detailFitur.js';
export default function DetailFiturDisplay(fitur) {
  let fiturDisplay = '';
  if (fitur === 'mandiri') {
    fiturDisplay = DetailFitur.mandiri;
  }
  if (fitur === 'aksesHP') {
    fiturDisplay = DetailFitur.aksesHP;
  }
  if (fitur === 'kompatibel') {
    fiturDisplay = DetailFitur.kompatibel;
  }
  console.log('fitur : ', fitur);
  console.log('fiturDisplay : ', fiturDisplay);

  const wrapperFitur = document.getElementById('detail-fitur');
  wrapperFitur.innerHTML = `    
    <!-- Modal content -->
    <div class="fixed inset-0 z-10 w-screen overflow-y-auto">
      <div
        class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0"
      >
        <div
          class="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg"
        >
          <div class="bg-white px-4 pb-4 pt-3 sm:p-6 sm:pb-4">
            <div class="sm:flex sm:items-start">
              <div class="mt-2 text-center sm:ml-4 sm:mt-0 sm:text-left">
                <h3
                  class="text-base font-semibold text-gray-900"
                  id="modal-title"
                >
                  Detail Fitur
                </h3>
                <div class="mt-2">${fiturDisplay}</div>
              </div>
            </div>
          </div>
          <div
            class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6"
          >
            <button
              id="cancelButton"
              class="mt-3 inline-flex w-full justify-center rounded-md bg-blue-600 px-3 py-2 text-sm font-semibold text-white shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-blue-800 sm:mt-0 sm:w-auto"
            >
              Close
            </button>
          </div>
        </div>
      </div>
    </div>
`;
  return wrapperFitur;
}
